...
public class PortletUriFragmentUtilityTest {
	...
	public void shouldSetInitialFragment() {
		...
		portletUriFragmentUtility.setInitialFragment(mainWindow);

		verify(mainWindow, times(1)).executeJavaScript(
				"window.location.hash = '" + fragment + "';");
	}

	@Test
	public void shouldRunFragmentChangedEvent() {
		FragmentChangedEvent source = mock(FragmentChangedEvent.class);

		UriFragmentUtility uriFragmentUtility = mock(UriFragmentUtility.class);
		when(uriFragmentUtility.getFragment()).thenReturn(fragment);
		when(source.getUriFragmentUtility()).thenReturn(uriFragmentUtility);

		portletUriFragmentUtility.fragmentChanged(source);

		verify(uriFragmentUtility, times(1)).getFragment();
	}
}
