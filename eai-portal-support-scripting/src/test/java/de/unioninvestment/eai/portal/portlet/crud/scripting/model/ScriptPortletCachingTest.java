...
public class ScriptPortletCachingTest {

	@Mock
	private PortletCaching delegateMock;
	private ScriptPortletCaching caching;
	
	@Before
	public void setUp() {
		MockitoAnnotations.initMocks(this);
		caching = new ScriptPortletCaching(delegateMock);
	}
	
	@Test
	public void shouldDelegateInvalidateQueryOptionList() {
		when(delegateMock.invalidateQueryOptionList("test", "select * from TESTTABLE")).thenReturn(true);
	...
	public void shouldDelegateInvalidateQueryOptionLists() {
		...
		when(delegateMock.invalidateQueryOptionLists("test", pattern)).thenReturn(4);
