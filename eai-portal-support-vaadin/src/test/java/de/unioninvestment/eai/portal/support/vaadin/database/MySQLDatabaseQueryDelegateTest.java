...
public class MySQLDatabaseQueryDelegateTest {
	private MySQLDatabaseQueryDelegate delegate;
	...
	public void setUp() {
		...
		delegate = new MySQLDatabaseQueryDelegate(QUERY,
				asList(new String[] { "ID" }));

		QueryBuilder.setStringDecorator(DatabaseDialect.MYSQL
				.getStringDecorator());
		prepareData();
	}
	...
	public void shouldCreateHelperWithCorrectCountStatement() {
		...
		assertEquals("SELECT COUNT(*) FROM (Select * from TestUser) c",
	...
	public void shouldRetrieveQueryString() {
		...
		assertEquals("Select * from TestUser", queryString);
	}
	...
	public void shouldRetrieveQueryStringOrderByASC() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c ORDER BY `id` ASC",
				queryString);
	}
	...
	public void shouldRetrieveQueryStringOrderByDESC() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c ORDER BY `id` DESC",
				queryString);
	}
	...
	public void shouldRetrieveQueryStringOrderByDESCAndASC() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c ORDER BY `id` DESC, `Name` ASC",
				queryString);
	}
	...
	public void shouldTestFilterBetween() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c WHERE `ID` BETWEEN ? AND ?",
				queryString);
	}
	...
	public void shouldRetrieveQueryStringNestedIntoOrderByNestedIntoWhere() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c WHERE `id` > ? ORDER BY `id` ASC",
				queryString);
	}
	...
	public void shouldRetrieveQueryStringNestedIntoOrderByNestedIntoWhereNestedIntoLimits() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c WHERE `id` > ? ORDER BY `id` ASC LIMIT 10,5",
				queryString);
	}
	...
	public void shouldTestFilterGreater() {
		...
		assertEquals("SELECT * FROM (Select * from TestUser) c WHERE `ID` > ?",
				queryString);
	}
	...
	public void shouldTestFilterGreaterAndEqual() {
		...
		assertEquals(
				"SELECT * FROM (Select * from TestUser) c WHERE `ID` > ? AND `SIZE` LIKE ?",
				queryString);
	}
	...
	public void shouldTestRightParentheses() {
		...
		MySQLDatabaseQueryDelegate scriptDatabaseQueryDelegate = new MySQLDatabaseQueryDelegate(
		...
		assertEquals(
				"SELECT * FROM (select OS_DEPLOY_LOG.*, case when error is null then 'Deployment successful' else 'Error' end MESSAGE from OS_DEPLOY_LOG  order by STARTDATE desc) c WHERE `MESSAGE` = ?",
				queryString);
	}
	...
	public void shouldGetIndexStatement() {
		...
		assertThat(
				queryString,
				is("SELECT c.`rownum` FROM (SELECT @rn:=@rn+1 `rownum`, a.* FROM ("
						+ "Select * from TestUser" // query
						+ ") a, (SELECT @rn:=0) r) c WHERE `ID`=?"));
	}

	@Test
	public void shouldGetIndexStatementWithOrderByAndFilters() {
		List<OrderBy> orderBys = new ArrayList<OrderBy>();
		orderBys.add(new OrderBy("NAME", true));
		delegate.setOrderBy(orderBys);
	...
	public void shouldGetIndexStatementWith2PK() {
		delegate = new MySQLDatabaseQueryDelegate(QUERY, asList(new String[] {
				"ID1", "ID2" }));
		prepareData();
		...
		assertThat(
				...
				is("SELECT c.`rownum` FROM (SELECT @rn:=@rn+1 `rownum`, a.* FROM ("
						+ "Select * from TestUser" // query
						+ ") a, (SELECT @rn:=0) r) c WHERE `ID1`=? AND `ID2`=?"));
	}
